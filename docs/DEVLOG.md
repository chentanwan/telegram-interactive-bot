...
## 2026-08-28　0.5 认领与快捷回复

多客服共用一个后台群时，没有「这单我来」的标记，两个人会同时回；欢迎语、审核中、下班时间这类句子每天复制。0.5 只做这两件值班工具，不碰锁单、不碰 Webhook。

### 认领是软的

`User.claimed_by` / `claimed_by_name` / `claimed_at`。`/claim` 覆盖上一个人，`/unclaim` 清空。其他人照样能回复——Telegram 群里锁不住输入框，做成硬锁只会制造「我回不了」的工单。`/info` 多一行认领，`/status` 多一个「认领中」计数。`/clear` 把认领一起清掉。

### 快捷回复怎么发

表 `canned_reply`，slug 1–32 位 `[a-zA-Z0-9_-]`。`/setreply hours 我们的工作时间是…`，也可以回复一条现成消息只写别名。`/reply hours` 必须在客户话题里：先在话题发一条（客服看得到谁用了哪条话术），再 copy 给客户，写入 MessageMap。所以这条快捷回复之后也能编辑同步、也能 `/del`。

`/replies` 在话题里会带按钮，callback `canned_<slug>`。别名和验证码 callback 前缀分开，避免撞车。

### 没做的

工作时间自动回复、按标签广播、认领超时提醒。那些要定时器和更多产品假设，留给以后。
